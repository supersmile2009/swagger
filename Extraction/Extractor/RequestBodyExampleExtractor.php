...
class RequestBodyExampleExtractor implements ExtractorInterface
{
    /**
     * @var MetadataFactory
     */
    ...
    {
        return $target instanceof MediaType
            && \is_string($source)
            && \class_exists($source)
            && $extractionContext->getParameter('in-model-context') !== null;
    }
     ...
     */
    public function extract($className, &$mediaType, ExtractionContextInterface $extractionContext)
    {
        if (!$this->canExtract($className, $mediaType, $extractionContext)) {
            return;
        }
        ...
        $serializationGroups = $modelContext['serializer-groups'] ?? null;

        $exclusionStrategies = [];

        if (null !== $serializationGroups) {
            $exclusionStrategies[] = new GroupsExclusionStrategy($serializationGroups);
        }
